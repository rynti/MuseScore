...
static constexpr playback::IPlaybackController::SeekOrigin USER_SEEK
    = playback::IPlaybackController::SeekOrigin::User;
...
TEST_F(NotationViewInputControllerTests, Mouse_Press_Range_Start_Drag_From_Selected_Element)
{
    ...
    //! [THEN] seek to the element before playing it
    {
        ...
        EXPECT_CALL(*m_playbackController, seekElement(newContext.element, USER_SEEK, FLUSH_SOUND))
        .Times(1);
...
TEST_F(NotationViewInputControllerTests, Mouse_Press_Range_Start_Play_From_First_Playable_Element)
{
    ...
    //! [THEN] seek to the element before playing it
    {
        ...
        EXPECT_CALL(*m_playbackController, seekElement(oldContext.element, USER_SEEK, FLUSH_SOUND))
        .Times(1);
...
TEST_F(NotationViewInputControllerTests, Mouse_Press_On_Already_Selected_Range)
{
    ...
    EXPECT_CALL(*m_playbackController, seekElement(context.element, USER_SEEK, FLUSH_SOUND))
    .Times(1);
...
TEST_F(NotationViewInputControllerTests, Mouse_Press_On_Range_Context_Menu)
{
    ...
    EXPECT_CALL(*m_playbackController, seekElement(selectMeasureContext.element, USER_SEEK, FLUSH_SOUND))
    .WillOnce(Return())
    //! right button click
    .WillOnce(Return());
...
TEST_F(NotationViewInputControllerTests, Mouse_Press_On_Range_Context_Menu_New_Selection)
{
    ...
    EXPECT_CALL(*m_playbackController, seekElement(contextMenuOnMeasureContext.element, USER_SEEK, FLUSH_SOUND))
    .WillOnce(Return());
